...
- `clap-validator` 0.4.1 (source commit `b2f1d9b79b1d264a5747f46707d72b1aa40a02ef`) reports 23 passed, 0 failed, and 21 intentional skips, including both double-precision process-audio cases;
- its five-second, two-worker fuzz run completed without errors, including malformed optional transport values;
- REAPER 7.79/macOS-arm64 scanned, instantiated, state-round-tripped, and rendered the current f64-capable artifact through a real project; the f32 differential render was bit-identical to 0.5× the no-FX render.
...
| REAPER 7.79 | macOS arm64 | pass | headless render of the current f64-capable artifact: scan, instantiate, state round trip, bit-exact f32 0.5-gain differential, bit-identical bypassed 64-bit float pass-through; f32 dispatch confirmed via subnormal probe (`data64` remains unexercised) |
